...
<input type = "submit" name="sub" class="btn" value="ADD CARD!"onmouseover="this.style.backgroundPosition='left -36px'"
...
<?php
 	...
 	if(!empty($_POST['sub'])){
		 ...
		 $sql = "INSERT INTO card VALUES('$id', '$name', '$dep','$st')";
// if the insert is null
		 if(!$id){
		 	echo "<script>alert('the ID cannot be null!');location.href = 'card_add.php'</script>"; 
		 }
		 else if(!$name){
		 	echo "<script>alert('the NAME cannot be null!');location.href = 'card_add.php'</script>"; 
		 }
		 else if(!$dep){
		 	echo "<script>alert('the DEPARTMENT cannot be null!');location.href = 'card_add.php'</script>"; 
		 }
		 else if(!$st){
		 	echo "<script>alert('the STU/TEA cannot be null!');location.href = 'card_add.php'</script>"; 
		 }
	 	 else{
			$check = mysql_query($sql_s);
			$rs_s = mysql_fetch_array($check);
			if($rs_s['id'] == $id){
				echo "<script>alert('this id has exist, choose another one!');location.href = 'card_add.php'</script>";
			}
			else{
					mysql_query($sql);
					echo "<script>alert('Add Card Success!');location.href = 'cards.php'</script>";  
				}
			}
		}

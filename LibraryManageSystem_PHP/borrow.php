...
<br>	
<h1>Books Borrowed & Check out</h1>
<br>
<br>
<p>Books Borrowed:</p>
<br>
...
<input type = "submit" name="sub2" class="btn" value="Borrow Books!"onmouseover="this.style.backgroundPosition='left -36px'"
 onmouseout="this.style.backgroundPosition='left top'">
 </form>
 <?php
	if(!empty($_POST['sub2'])){
 		$borrow = $_POST['borrow'];
 		$sql_b = "SELECT * FROM books WHERE id = '$borrow'";
 		$query_b = mysql_query($sql_b);
		$rs_b = mysql_fetch_array($query_b);

		$sql_r = "SELECT * FROM record WHERE book_id = '$borrow' order by return_time desc";
 		$query_r = mysql_query($sql_r);
		$rs_r = mysql_fetch_array($query_r);
		$admin = $_COOKIE['admincookie'];
		$card = $_COOKIE['cardcookie'];
		$time_b = date("Y-m-d H:i:s");
		$time_r = date('Y-m-d H:i:s',strtotime("+2 month"));

		if(!$rs_b){
			echo "<script>alert('Book doesn\'t exit!!');location.href = 'borrowpre.php'</script>";
		}
		else if($rs_b['number'] == 0){
			$test = $rs_r['return_time'];
			echo "<script>alert('Sorry, No Books you have chooesn is available, The latest book will be returned in: '+'$test');location.href = 'borrowpre.php'</script>";
		}
		else{
			$sql_success = "UPDATE books SET number = number - 1 where id ='$borrow'";
			mysql_query($sql_success);
			$sql_successre = "INSERT INTO record VALUES('$borrow', '$card', '$time_b', '$time_r', '$admin')";
			mysql_query($sql_successre);
			echo "<script>alert('Borrow Successfully!!');location.href = 'borrowpre.php'</script>";
		}
	}
 ?>
<br>

...
content<br><textarea rows="5" cols ="50" name = "add2"> </textarea><br> 
...
// if the insert is null
		 if(!$id){
		 //	mysql_query($sql_d);
		 	echo "<script>alert('the ID cannot be null!');location.href = 'add2.php'</script>"; 
		 }
		 else if(!$category){
		 //	mysql_query($sql_d);
		 	echo "<script>alert('the CATEGORY cannot be null!');location.href = 'add2.php'</script>"; 
		 }
		  else if(!$title){
		 //	mysql_query($sql_d);
		 	echo "<script>alert('the TITLE cannot be null!');location.href = 'add2.php'</script>"; 
		 }
		  else if(!$press){
		 //	mysql_query($sql_d);
		 	echo "<script>alert('the PRESS cannot be null!');location.href = 'add2.php'</script>"; 
		 }

		 else if(!$time){
		 //	mysql_query($sql_d);
		 	echo "<script>alert('the TIME cannot be null!');location.href = 'add2.php'</script>"; 
		 }
		  else if(!$author){
		 //	mysql_query($sql_d);
		 	echo "<script>alert('the AUTHOR cannot be null!');location.href = 'add2.php'</script>"; 
		 }
		  else if(!$price){
		 //	mysql_query($sql_d);
		 	echo "<script>alert('the PRICE cannot be null!');location.href = 'add2.php'</script>"; 
		 }
		  else if(!$quantity){
		 //	mysql_query($sql_d);
		 	echo "<script>alert('the QUANTITY cannot be null!');location.href = 'add2.php'</script>"; 
		 }
		 //
	 	else{
...
//if the id is exist.. 
			if($rs['id'] == $id){
				if($rs['category']!=$category||$rs['name']!=$title||$rs['press']!=$press||$rs['time']!=$time||$rs['author']!=$author||$rs['price']!=$price){
					echo "<script>alert('this book id has exist, choose another one!');location.href = 'add2.php'</script>";  
				}
				else{
					mysql_query($sql_u);
	...
	echo "<script>alert('add new book success!');location.href = 'add2.php'</script>"; 
